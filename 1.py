...
def draw_lines(frame, vek_list):
	for vek in vek_list:
		cv.line(frame, vek[0], vek[1], (0,100,255), 3, cv.LINE_AA)
		frame = cv.circle(frame, vek[0], 4, (255, 0, 0), 2)
		frame = cv.circle(frame, vek[1], 4, (0, 0, 255), 2)			
	return frame

				# direction = ((
					# int((vek[0][0][0] + vek[1][0][0]) / 2),
					# int((vek[0][0][1] + vek[1][0][1]) / 2)),
					# (
					# int((vek[0][1][0] + vek[1][1][0]) / 2),
					# int((vek[0][1][1] + vek[1][1][1]) / 2))
				# )
